...
语言切换、亮/暗主题始终固定在每一页最顶部。文章目录在右侧，做成类似 Time Machine / Codex 的刻度滑轮。顶栏「埋点」看这台浏览器留下的打开、搜索和停留。默认只存在本地；若要外发，设环境变量 `NEXT_PUBLIC_ANALYTICS_ENDPOINT`。GitHub Pages 上的站点不需要登录。
